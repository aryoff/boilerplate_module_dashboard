...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            if ($key == 0) {
                $sisa = DB::connection('mirror')->select("SELECT rekap->'sisa' AS sisa FROM dynamicticket_rekap_campaign_daily WHERE tanggal=CURRENT_DATE AND dynamicticket_escalation_campaign_id=:escalation_campaign_id;", ['escalation_campaign_id' => $value->dynamicticket_escalation_campaign_id]);
